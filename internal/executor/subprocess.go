...
import (
	...
	mcptypes "github.com/sudebaker/journalist-mcp/internal/mcp"
...
func (e *Executor) Execute(ctx context.Context, toolName string, arguments map[string]interface{}) (*ExecuteResult, error) {
	...
	// When context is cancelled, send SIGTERM first, SIGKILL after 5s grace period
	cmd.Cancel = func() error {
		if cmd.Process != nil {
			log.Debug().Str("tool", toolName).Msg("Sending SIGTERM to subprocess")
